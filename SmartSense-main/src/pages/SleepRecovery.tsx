...
// sends ONE real 32-feature Rest Mode row to the FastAPI ML backend
// (finalized XGBoost N2 classifier, running locally) and this page shows the
// prediction it returns, in the same Sleep State / "How you're resting" UI
// that's already here. Stage 1 has no ESP32/BioAmp wearable yet, so the
// feature vector comes from a real DREAMT test-set row bundled at build time
// (src/lib/demoSamples/restSamples.json) rather than a live 8-second window;
...
export default function SleepRecovery() {
  ...
  // Root cause of the "shows N2 while the live model actually predicted Non-N2" bug: this used to
  // fall through to `state.sleepState` (the SIMULATION-only random-walk field, defaulting to "N2" in
  // initialState) whenever `analysis` was null — which it always is in LIVE mode, since the manual
  // "Analyze Sleep (sample data)" button only exists in the SIMULATION branch below. That left the
  // top Sleep State card and "How you're resting" headline frozen on stale simulated state while the
  // real backend prediction (state.liveRestPrediction) was already correct and shown further down in
  // the separate "Live sleep state" panel — the same prediction, disagreeing with itself on one page.
  // LIVE must only ever be driven by the authoritative WS prediction, never by SIMULATION fields.
  const displaySleepState: SleepState =
    state.dataSource === "LIVE"
      ? state.liveRestPrediction
        ? state.liveRestPrediction.prediction === "N2"
          ? "N2"
          : "NON_N2"
        : "WAKE"
      : analysis
        ? (analysis.prediction === "N2" ? "N2" : "NON_N2")
        : state.sleepState;
  ...
  return (
    <DashboardShell>
      ...
      <section className="grid gap-6 xl:grid-cols-7">
        <Card className="xl:col-span-4">
          ...
          <p className="mt-1 font-display text-lg font-bold">
            {displaySleepState === "N2" ? "You've reached N2 — restful and stable." : displaySleepState === "NON_N2" ? "You're in Non-N2 sleep." : "You're still settling in."}
          </p>
          <div className="mt-5">
            ...
            <div className="mt-5 border-t border-border/60 pt-5">
              <Button variant="secondary" onClick={runAnalysis} disabled={analyzing}>
                {analyzing ? (
                  ...
                  <>
                    Analyze Sleep (sample data) <Moon size={15} />
                  </>
                )}
              </Button>
          ...
          {state.dataSource === "LIVE" && (
            <div className="mt-5 border-t border-border/60 pt-5">
              <SectionLabel>Live sleep state</SectionLabel>
              {!state.liveBackendReachable || state.liveConnection !== "CONNECTED" ? (
                <p className="mt-2 text-sm text-muted-foreground">Monitoring…</p>
              ) : state.liveQuality === "POOR" ? (
                <p className="mt-2 text-sm text-muted-foreground">Monitoring…</p>
              ) : state.liveRestPrediction ? (
                <div className="mt-2">
                  <p className="text-sm">
                    <b>{state.liveRestPrediction.prediction}</b> · {(state.liveRestPrediction.n2Probability * 100).toFixed(1)}% N2 probability ·
                    smoothed state <b>{state.liveRestPrediction.smoothedState}</b> ({state.liveRestPrediction.consecutiveN2} consecutive N2 epochs)
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Updated {new Date(state.liveRestPrediction.timestamp * 1000).toLocaleTimeString()} · one prediction per real 30s epoch
                  </p>
                </div>
              ) : (
                <p className="mt-2 text-sm text-muted-foreground">Buffering the first 30-second epoch — a prediction appears once it's complete.</p>
              )}
            </div>
          )}
        </Card>
      ...
      <p className="flex items-center gap-2 text-xs text-muted-foreground">
        <Moon size={13} />{" "}
        {state.dataSource === "LIVE"
          ? "Sleep state shown here is the live prediction from the ML backend, updated once per real 30-second epoch."
          : "Sleep insights are simulated for this demo and update automatically while you rest."}
      </p>
    </DashboardShell>
  );
}
