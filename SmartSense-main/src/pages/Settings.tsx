            ...
            <div className="mt-3 flex items-center gap-3 rounded-2xl bg-secondary/40 p-3.5">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold">
                  {state.liveConnection === "CONNECTED" ? "ESP32/BioAmp backend connected" : state.liveConnection === "CONNECTING" ? "Connecting to ML backend…" : "No live data yet"}
                </p>
                <p className="text-xs text-muted-foreground">
                  {state.liveConnection === "CONNECTED"
                    ? `Detected sample rate: ${state.liveDetectedFs ? state.liveDetectedFs.toFixed(1) : "?"} Hz · see Live Monitor for signal detail`
                    : "Waiting for the backend's live stream (see Live Monitor) — nothing is faked while this shows."}
                </p>
              </div>
              <Pill tone={state.liveConnection === "CONNECTED" ? "good" : "warn"}>
                {state.liveConnection === "CONNECTED" ? "Connected" : state.liveConnection === "CONNECTING" ? "Connecting…" : "Waiting…"}
              </Pill>
            </div>
          )}
        </div>
