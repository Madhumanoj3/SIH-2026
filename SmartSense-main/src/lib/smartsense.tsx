...
import { isSupabaseConfigured, getDbIdentity, startDbSession, endDbSession, persistRestEpoch, type DbStatus } from "./dbSession";
...
/** Live ESP32/BioAmp backend — same host as the FastAPI REST endpoints used by the
 * Simulation-mode "Analyze" demo buttons (SleepRecovery.tsx / Drowsiness.tsx). */
export const LIVE_BACKEND_WS_URL = "ws://127.0.0.1:8000";
...
/** Live-mode-only connection state — separate from the always-on `eegStatus`/`eogStatus` pair (which
 * LIVE mode also updates for backwards compatibility with pages that only look at those). */
export type LiveConnectionState = "DISCONNECTED" | "CONNECTING" | "CONNECTED";
export type LiveSignalQuality = "GOOD" | "FAIR" | "POOR" | "UNKNOWN";

export interface LiveRestPrediction {
  prediction: "N2" | "Non-N2";
  n2Probability: number;
  smoothedN2Probability: number;
  smoothedState: "N2" | "Non-N2";
  consecutiveN2: number;
  smartAlarm: boolean;
  timestamp: number;
}

export interface LiveDrivePrediction {
  vigilance: number;
  trendLabel: string;
  status: string;
...
/** Display labels for the Rest Mode N2 vs Non-N2 classifier. Only these two terms are ever shown for
 * a sleep-stage result — no N1/N3/REM/"deep sleep"/"light sleep" wording, since the model is a
 * binary N2/Non-N2 classifier, not a 4-stage sleep-stage detector. WAKE is a distinct pre-rest
 * app state (not resting yet), not a sleep-stage claim. */
...
/** Wake Up Window shown on Sleep/Recovery: always the browser's current local time through exactly
 * 30 minutes later, formatted 12-hour with AM/PM (e.g. "10:15 PM – 10:45 PM"). Recomputed fresh
 * every time it's needed — never a fixed/old configured window, never 1 hour or any other span.
 * Formatted manually (not via toLocaleTimeString) so AM/PM is always uppercase regardless of the
 * browser's locale/ICU data — some locales/environments render it lowercase ("10:15 pm"). */
...
export function computeWakeWindow(): { start: string; end: string } {
  const start = new Date();
  const end = new Date(start.getTime() + 30 * 60000);
  return { start: formatClock12h(start), end: formatClock12h(end) };
}
...
export interface SmartSenseState {
  ...
  /** LIVE mode (ESP32 + BioAmp -> backend, see lib/liveClient.ts) — populated only while
   * dataSource === "LIVE"; SIMULATION never touches these fields. */
  /** Whether the WebSocket to the backend itself is open — independent of liveConnection, which
   * reflects whether the ESP32/BioAmp is actually sending data through that backend. Lets the UI
   * distinguish "ML BACKEND OFFLINE" from "WAITING FOR DATA" instead of collapsing both to one state. */
  liveBackendReachable: boolean;
  liveConnection: LiveConnectionState;
  liveQuality: LiveSignalQuality;
  liveDetectedFs: number | null;
  liveEeg: number[];
  liveEog: number[];
  liveLastSampleAt: number | null;
  liveRejectReason: string | null;
  /** Set true the moment a sequence gap discards a window, cleared once the next valid prediction
   * arrives — lets the UI show real recovery progress instead of a frozen POOR state that looks
   * identical whether the system is correctly waiting or actually broken. */
  liveRecovering: boolean;
  liveContiguousSamples: number;
  liveTargetSamples: number;
  liveRestPrediction: LiveRestPrediction | null;
  liveDrivePrediction: LiveDrivePrediction | null;
}
...
type Action =
  ...
  | { type: "liveStatus"; connection: LiveConnectionState; detectedFs?: number | null; recovering?: boolean; contiguousSamples?: number; targetSamples?: number }
  | { type: "liveBackendReachable"; reachable: boolean }
  | { type: "liveSample"; eeg: number; eog: number; t: number }
  | { type: "liveRestPrediction"; data: LiveRestPrediction }
  | { type: "liveDrivePrediction"; data: LiveDrivePrediction }
  | { type: "liveQualityReject"; reason: string; recovering?: boolean; targetSamples?: number }
  | { type: "liveReset" };
...
export const initialState: SmartSenseState = {
  ...
  // Real "now -> now+30min" even as a default, not a stale hardcoded demo time — see
  // computeWakeWindow(). Becomes fully live the moment a rest session actually begins.
  wakeWindowStart: computeWakeWindow().start,
  wakeWindowEnd: computeWakeWindow().end,
  ...
  liveBackendReachable: false,
  liveConnection: "DISCONNECTED",
  liveQuality: "UNKNOWN",
  liveDetectedFs: null,
  liveEeg: [],
  liveEog: [],
  liveLastSampleAt: null,
  liveRejectReason: null,
  liveRecovering: false,
  liveContiguousSamples: 0,
  liveTargetSamples: 0,
  liveRestPrediction: null,
  liveDrivePrediction: null,
};
...
export function reduceState(state: SmartSenseState, action: Action): SmartSenseState {
  switch (action.type) {
    case "tick": {
      ...
      // depends on the simulated sleep state during the minute that just passed (N2 recovers
      // fastest, Non-N2 slower, still-settling-in barely at all). This is what makes
      ...
      if (state.operatingMode === "REST") {
        ...
        // Wake Up Window shown to the user is always "now -> now + 30 minutes", recomputed live
        // every tick using the browser's local clock — a deliberate product/UI simplification, not
        // a change to the underlying rest-readiness detection (consecutiveN2Epochs/wakeStatus above,
        // which is what actually decides when a wake is genuinely appropriate, is untouched).
        const window = computeWakeWindow();
        wakeWindowStart = window.start;
        wakeWindowEnd = window.end;
      }
      return {
        ...state,
    ...
    case "arrive": {
      ...
      return {
        ...
        // Set immediately on entry (not just on the next tick) so there's no brief flash of the
        // stale demo default window before the first tick recomputes it.
        wakeWindowStart: window.start,
        wakeWindowEnd: window.end,
      };
    }
    ...
    case "liveStatus": {
      const connected = action.connection === "CONNECTED";
      return {
        ...state,
        liveConnection: action.connection,
        liveDetectedFs: action.detectedFs !== undefined ? action.detectedFs : state.liveDetectedFs,
        liveRecovering: action.recovering !== undefined ? action.recovering : state.liveRecovering,
        liveContiguousSamples: action.contiguousSamples !== undefined ? action.contiguousSamples : state.liveContiguousSamples,
        liveTargetSamples: action.targetSamples !== undefined ? action.targetSamples : state.liveTargetSamples,
        eegStatus: connected ? "CONNECTED" : "DISCONNECTED",
        eogStatus: connected ? "CONNECTED" : "DISCONNECTED",
        eegQuality: connected ? state.eegQuality : 0,
        eogQuality: connected ? state.eogQuality : 0,
      };
    }
    case "liveBackendReachable":
      return { ...state, liveBackendReachable: action.reachable };
    case "liveSample":
      return {
        ...state,
        liveEeg: [...state.liveEeg.slice(-199), action.eeg],
        liveEog: [...state.liveEog.slice(-199), action.eog],
        liveLastSampleAt: action.t,
      };
    case "liveRestPrediction": {
      // Rising edge only — fires once when the smart alarm first trips, never once per epoch it stays true.
      let alerts = state.alerts;
      if (action.data.smartAlarm && !state.liveRestPrediction?.smartAlarm) {
        alerts = pushAlert(state, { titleKey: "alertWakeReadyTitle", bodyKey: "alertWakeReadyBody", severity: "good", category: "rest" });
      }
      return {
        ...state,
        liveRestPrediction: action.data,
        liveQuality: "GOOD",
        liveRejectReason: null,
        liveRecovering: false,
        alerts,
    ...
    case "liveDrivePrediction": {
      let alerts = state.alerts;
      let safetyStage = state.safetyStage;
      if (action.data.sustainedDecline && !state.liveDrivePrediction?.sustainedDecline) {
        alerts = pushAlert(state, { titleKey: "alertSevereTitle", bodyKey: "alertSevereBody", severity: "danger", category: "drowsiness" });
        safetyStage = "ALERT";
      }
      return {
        ...state,
        liveDrivePrediction: action.data,
        liveQuality: "GOOD",
        liveRejectReason: null,
        liveRecovering: false,
        alerts,
    ...
    case "liveQualityReject":
      return {
        ...state,
        liveQuality: "POOR",
        liveRejectReason: action.reason,
        liveRecovering: action.recovering !== undefined ? action.recovering : state.liveRecovering,
        liveContiguousSamples: action.recovering ? 0 : state.liveContiguousSamples,
        liveTargetSamples: action.targetSamples !== undefined ? action.targetSamples : state.liveTargetSamples,
      };
    case "liveReset":
      return {
...
export function SmartSenseProvider({ children }: { children: ReactNode }) {
  ...
  // Per-session running epoch counter for sleep_epochs.epoch_number (unique per session_id) — reset
  // to 0 every time a fresh sleep_sessions row starts, incremented once per persisted Rest epoch.
  const restEpochCounterRef = useRef(0);
  ...
  // --- LIVE mode: exactly one WebSocket, owned here --------------------
  // Lifted to the provider (rather than per-page) so there is only ever one
  // connection regardless of which page is open, keyed off operatingMode
  // (REST -> /api/live/stream/rest, DRIVING -> /api/live/stream/drive). The
  // effect's cleanup always runs before the next one fires, so switching
  // dataSource or operatingMode can never leave a duplicate/stale socket
  // open. SIMULATION mode never opens a socket at all.
  useEffect(() => {
    if (state.dataSource !== "LIVE") return;
    const mode = state.operatingMode === "REST" ? "rest" : "drive";
    let closedByEffect = false;

    dispatch({ type: "liveStatus", connection: "CONNECTING" });
    const ws = new WebSocket(`${LIVE_BACKEND_WS_URL}/api/live/stream/${mode}`);

    // Socket-open only means "the backend itself is reachable" — it does NOT mean the ESP32 is
    // sending data. liveConnection (CONNECTED/DISCONNECTED) is only ever set from the backend's own
    // status messages below, which reflect real recent-sample recency, never guessed here.
    ws.onopen = () => dispatch({ type: "liveBackendReachable", reachable: true });
    ws.onerror = () => {
      dispatch({ type: "liveBackendReachable", reachable: false });
      dispatch({ type: "liveStatus", connection: "DISCONNECTED" });
    };
    ws.onclose = () => {
      if (!closedByEffect) {
        dispatch({ type: "liveBackendReachable", reachable: false });
    ...
    ws.onmessage = (event) => {
      ...
      switch (msg.type) {
        case "sample":
          dispatch({ type: "liveSample", eeg: msg.eeg as number, eog: msg.eog as number, t: msg.t as number });
          break;
        case "status": {
          const detectedFs = mode === "rest" ? (msg.detected_fs_rest as number | null) : (msg.detected_fs_drive as number | null);
          const recovering = mode === "rest" ? (msg.rest_recovering as boolean | undefined) : (msg.drive_recovering as boolean | undefined);
          const contiguousSamples = mode === "rest" ? (msg.rest_contiguous_samples as number | undefined) : (msg.drive_contiguous_samples as number | undefined);
          const targetSamples = mode === "rest" ? (msg.rest_target_samples as number | undefined) : (msg.drive_target_samples as number | undefined);
          dispatch({
            type: "liveStatus",
            connection: msg.connection === "CONNECTED" ? "CONNECTED" : "DISCONNECTED",
            detectedFs,
            recovering,
            contiguousSamples,
            targetSamples,
        ...
        case "prediction":
          if (msg.mode === "rest") {
            dispatch({
              type: "liveRestPrediction",
              data: {
                prediction: msg.prediction as "N2" | "Non-N2",
                n2Probability: msg.n2_probability as number,
                smoothedN2Probability: msg.smoothed_n2_probability as number,
                smoothedState: msg.smoothed_state as "N2" | "Non-N2",
                consecutiveN2: msg.consecutive_n2 as number,
                smartAlarm: msg.smart_alarm as boolean,
                timestamp: msg.timestamp as number,
              },
            });
            // Persist the SAME authoritative message just dispatched above — never a
            // frontend-recomputed value — as one sleep_epochs row, only while a real DB session is
            // open (i.e. actually resting with Supabase configured). Fire-and-forget: a DB hiccup
            // must never block/delay the on-screen live prediction.
            if (isSupabaseConfigured && dbSessionIdRef.current) {
              restEpochCounterRef.current += 1;
              void persistRestEpoch({
                sessionId: dbSessionIdRef.current,
                epochNumber: restEpochCounterRef.current,
                timestamp: msg.timestamp as number,
                prediction: msg.prediction as "N2" | "Non-N2",
                n2Probability: msg.n2_probability as number,
                smoothedN2Probability: msg.smoothed_n2_probability as number,
          ...
          } else if (msg.mode === "drive") {
            dispatch({
              type: "liveDrivePrediction",
              data: {
                vigilance: msg.vigilance as number,
                trendLabel: msg.trend_label as string,
                status: msg.status as string,
                sustainedDecline: msg.sustained_decline as boolean,
                timestamp: msg.timestamp as number,
              },
            });
          }
        ...
        case "quality_reject":
          dispatch({
            type: "liveQualityReject",
            reason: msg.reason as string,
            recovering: msg.recovering as boolean | undefined,
            targetSamples: msg.target_samples as number | undefined,
          });
          break;
      }
    };
    ...
    return () => {
      closedByEffect = true;
      ws.close();
      dispatch({ type: "liveReset" });
    };
  }, [state.dataSource, state.operatingMode]);
